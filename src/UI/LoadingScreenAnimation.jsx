...
function SpinningShape() {
  ...
  useFrame((state) => {
    if (meshRef.current) {
      ...
      meshRef.current.rotation.y = state.clock.getElapsedTime() * 1.8
    }
    if (matRef.current) {
      matRef.current.emissiveIntensity =
        0.6 + Math.sin(state.clock.getElapsedTime() * 4) * 0.3
    }
  })
...
const bootMessages = [
  "Initializing Soumya Purani v3.0.0",
...
function LoadingScreenAnimation({ onComplete }) {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      currentProg = Math.min(currentProg + 3, 100);
      setProgress(currentProg);
      if (currentProg >= 100) progressDone = true;

      const msgIdx = Math.min(Math.floor(currentProg / 14), bootMessages.length - 1);
      setCurrentMessageIndex(msgIdx);

      const snippet = codeSnippets[snipIdx];
      const typingDone = snipIdx >= codeSnippets.length - 1 && charIdx >= snippet.length;

      if (pauseRemaining > 0) {
        pauseRemaining--;
      } else if (charIdx >= snippet.length) {
        if (snipIdx < codeSnippets.length - 1) {
          snipIdx++;
          charIdx = 0;
          lines = [...lines, ''];
        ...
        lines[lines.length - 1] += snippet[charIdx];
        charIdx++;
      }
     ...
     <div className="relative z-10 p-6 flex flex-col h-full" style={{ minHeight: '420px' }}>
      ...
      <div className="mb-5 space-y-1.5 text-xs">
       ...
       <div className="flex items-start gap-2" style={{ color: 'var(--text-color)' }}>
        ...
        <div
          className="mb-5 rounded-lg p-3 text-xs space-y-1.5"
          style={{
        ...
        >
          {displayedLines.map((line, i) => (
            <div key={i} className="flex items-start gap-2">
              <span className="select-none shrink-0" style={{ color: 'var(--accent-color)' }}>{i + 1}</span>
